...
def grouplookup(connection, metadata, type_id):
    ...
    try:
        ...
        print("Group lookup failed on typeid {}".format(type_id))
        groupid = -1
...
def importyaml(connection, metadata, source_path):
    ...
    regions = glob.glob(
        os.path.join(
            source_path, 'fsd', 'universe', '*', '*', 'region.staticdata'
        )
    )
    ...
    for regionfile in regions:
        ...
        if region.get('wormholeClassID'):
            connection.execute(
                mapLocationWormholeClasses.insert(),
                locationID=region['regionID'],
                wormholeClassID=region['wormholeClassID']
            )

        print("Importing Constellations.")
        constellations = glob.glob(
            os.path.join(head, '*', 'constellation.staticdata')
        )
        for constellationfile in constellations:
            ...
            systems = glob.glob(
                os.path.join(chead, '*', 'solarsystem.staticdata')
            )
            ...
            for systemfile in systems:
                ...
                for stargate in system.get('stargates', []):
                    jump = {
                        'stargateID': stargate,
                        'destinationID': system['stargates'][stargate]['destination']
                    }
                ...
                for planet in system.get('planets'):
                    ...
                    for npcstation in system['planets'][planet].get('npcStations', []):
                        # TODO Collect and store radius for stations,
                        # would be helpful for docking/jump routes
...
def fixStationNames(connection, metadata):
    ...
    connection.execute(
        staStations.update().values(
            stationName=select(
                [invNames.c.itemName]
            ).where(staStations.c.stationID == invNames.c.itemID)
        )
    )
